...
const getSeatGrade = (seatNumber) => {
  ...
  if (['A', 'B', 'C'].includes(row)) return { label: 'VIP', color: '#C9A84C' };
  if (['D', 'E', 'F', 'G'].includes(row)) return { label: 'R', color: '#6B7280' };
  return { label: 'S', color: '#92623B' };
};
            ...
            return (
              <div
                ...
                style={{
                  ...
                  backgroundColor: isSelected ? '#2563eb' : isReserved ? '#e5e5e5' : grade.color + '33',
                  ...
                  boxShadow: isSelected ? '0 4px 14px rgba(37,99,235,0.40)' : 'none',
                }}
              >
                {seat.seatNumber}
              </div>
        ...
        <div style={{ borderTop: '1px solid #ebebeb', paddingTop: '36px' }}>
          {selectedSeat && (
            <div style={{ display: 'inline-flex', gap: '24px', alignItems: 'center', backgroundColor: '#fff', border: '1px solid #ebebeb', borderRadius: '14px', padding: '18px 32px', marginBottom: '24px', fontSize: '15px' }}>
              ...
              <span style={{ fontWeight: '900', color: '#2563eb', fontSize: '18px' }}>{selectedSeat.price?.toLocaleString()}원</span>
            </div>
          )}

          <div style={{ display: 'flex', justifyContent: 'center', gap: '12px' }}>
            <button onClick={() => navigate(isChangeMode ? '/mypage' : `/events/${eventId}`)}
            ...
            <button onClick={handleReservation} disabled={!selectedSeat} className="vivid-button"
